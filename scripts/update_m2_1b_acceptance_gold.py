"""Check or explicitly update canonical Milestone 2.1B partial gold."""
...
REPOSITORY = FIXTURE_ROOT / "m2-1b-repo"
GOLD_PATH = FIXTURE_ROOT / "m2-1b-graph.json"
...
def generated_graph() -> str:
    ...
    if canonical_index_json(validated) != rendered:
        raise ValueError("canonical round-trip changed M2.1B partial output")
    return rendered
...
def difference(expected: str, actual: str) -> str:
    return "".join(
        difflib.unified_diff(
            ...
            fromfile="typescript_frontend/m2-1b-graph.json",
            tofile="typescript_frontend/generated-m2-1b-graph.json",
        )
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--update",
        action="store_true",
        help="explicitly replace committed M2.1B gold after reviewing semantic changes",
    )
    ...
    if expected != rendered:
        ...
        print("M2.1B gold differs; review the diff, then run with --update if intentional.")
        return 1
    print("M2.1B partial gold matches")
    return 0
