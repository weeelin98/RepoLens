...
def test_default_scanner_accepts_exact_js_family_suffixes(
    tmp_path: Path,
) -> None:
    ...
    assert tuple(source.suffix for source in result.files) == (".ts", ".js", ".jsx", ".tsx")


def test_js_family_still_obeys_ignore_and_resource_limits(tmp_path: Path) -> None:
    write_file(tmp_path, ".gitignore", "ignored.tsx\n")
    write_file(tmp_path, "ignored.tsx", "ignored")
    write_file(tmp_path, "a.js", "a")
    write_file(tmp_path, "b.jsx", "b")
    write_file(tmp_path, "c.tsx", "c")

    result = scan_repository(tmp_path, RuntimeConfig(maximum_file_count=2))

    assert result_paths(result) == ("a.js", "b.jsx")
    assert diagnostic_pairs(result) == (("c.tsx", ScanDiagnosticCode.FILE_COUNT_LIMIT_REACHED),)
